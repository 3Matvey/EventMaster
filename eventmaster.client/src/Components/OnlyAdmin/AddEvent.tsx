   ...
   // const dispatch = useDispatch();
    ...
    const handleAddEvent = async () => {
        ...
        try {
            ...
            const response = await axios.post(`${Server_HostAddress}/api/events/create_event`, newEvent, {
                headers: {
    ...
    const uploadImageUrl = async (eventId: number, imageUrl: string) => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.post(`${Server_HostAddress}/api/events/${eventId}/upload-image`, { imageUrl }, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
                },
    ...
    const handleImageFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        setImageUrl(''); // Очистить URL изображения при выборе файла
        setImageFile(e.target.files ? e.target.files[0] : null);
    };

    // Обработка ввода URL изображения
    const handleImageUrlChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        setImageFile(null); // Очистить файл при вводе URL
