...
import './ShowEvent.css'; // Импортируем стили

export function ShowEvent() {
    const { eventId } = useParams<{ eventId: string }>(); // Получаем имя события из URL
    const [event, setEvent] = useState<EventDetails | null>(null);
    const [participants, setParticipants] = useState<User[]>([]); // Список участников
    const userRole = localStorage.getItem('role'); // Роль пользователя для проверки прав администратора
